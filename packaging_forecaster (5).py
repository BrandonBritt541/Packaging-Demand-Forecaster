...
# How many recent weeks to hold out and test against.
test_weeks = st.sidebar.number_input("Backtest: weeks to hold out",
                                     min_value=4, max_value=26, value=12)

# Split: train on everything EXCEPT the last `test_weeks`, then predict them.
train = ts.iloc[:-test_weeks]
actual = ts.iloc[-test_weeks:]

with st.spinner("Running backtest..."):
    sf_bt = StatsForecast(models=[AutoARIMA(season_length=52)], freq="W")
    sf_bt.fit(train[["unique_id", "ds", "y"]])
    bt = sf_bt.predict(h=test_weeks)

# Align predictions with what actually happened and score the error.
pred_vals = bt["AutoARIMA"].values
true_vals = actual["y"].values

# MAPE = average % difference between forecast and reality (lower is better).
mape = np.mean(np.abs((true_vals - pred_vals) / true_vals)) * 100
# RMSE = typical error size in units.
rmse = np.sqrt(np.mean((true_vals - pred_vals) ** 2))

b1, b2 = st.columns(2)
b1.metric("Forecast error (MAPE)", f"{mape:.1f}%")
b2.metric("Typical error (RMSE)", f"{rmse:,.0f} units")

# Plot: actual vs what the model would have predicted for the held-out weeks.
fig3 = go.Figure()
fig3.add_trace(go.Scatter(x=train["ds"], y=train["y"],
                          mode="lines", name="Training history",
                          line=dict(color="#4C9BE8")))
fig3.add_trace(go.Scatter(x=actual["ds"], y=true_vals,
                          mode="lines", name="Actual (held out)",
                          line=dict(color="#2ECC71")))
fig3.add_trace(go.Scatter(x=actual["ds"], y=pred_vals,
                          mode="lines", name="Backtest forecast",
                          line=dict(color="orange", dash="dash")))
fig3.update_layout(xaxis_title="Week", yaxis_title="Units", height=420)
st.plotly_chart(fig3, use_container_width=True)

st.caption(
    f"The model was trained without the last {test_weeks} weeks, then asked to "
    "predict them. Green is what actually happened; orange dashed is the forecast. "
    f"A MAPE of {mape:.1f}% means forecasts were off by that much on average \u2014 "
    "under ~15% is generally considered good for demand forecasting."
)
